...
// by Ingemar Ragnemalm 2005, 2008, 2019
...
void CenterModel(Model *m)
{
	...
	for (i = 0; i < m->numVertices; i++)
	{
		m->vertexArray[i].x -= (maxx + minx)/2.0;
		m->vertexArray[i].y -= (maxy + miny)/2.0;
		m->vertexArray[i].z -= (maxz + minz)/2.0;
	}
}
...
// Added 2012, modified 2013, to do decent error reporting

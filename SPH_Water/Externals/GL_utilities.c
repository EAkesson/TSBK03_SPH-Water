...
void printError(const char *functionName)
{
   ...
   while (( error = glGetError() ) != GL_NO_ERROR)
   {
       if ((lastError != error) || (strcmp(functionName, lastErrorFunction)))
       {
	       ...
	       strcpy_s(lastErrorFunction, sizeof(lastErrorFunction), functionName);
	       lastError = error;
       }
   }
}
...
// override function using texturedata aswell
FBOstruct *initFBOTexture(int width, int height, int int_method, GLbyte* textureData)
{
	FBOstruct *fbo = malloc(sizeof(FBOstruct));
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, textureData);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, fbo->texid, 0);
